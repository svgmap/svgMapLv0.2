...
class SvgMap {
	...
	//	console.log(mapCanvas);
		...
		if (!this.#mapViewerProps.hasMapCanvasSize()){
			...
			setTimeout(function(){this.#initLoad()}.bind(this),50);
	...
	//				console.log("parentElem:",parentElem);
				...
				this.#svgImagesProps[id].rootLayer = this.#svgImagesProps[parentSvgDocId].rootLayer;
				parentElem.setAttribute("class" , "rootLayer:"+ this.#svgImagesProps[parentSvgDocId].rootLayer);
			}
		}
	}
	...
	//		console.log("call loadSVG  create svgImagesProps id:",id);
			this.#svgImagesProps[id] = new function(){}; //  2014.5.27
	...
	//		var httpObj = createXMLHttpRequest( function(){ return handleResult(id , path , parentElem , this); } );
			var httpObj = this.#createXMLHttpRequest( 
				function(){ that.#handleResult(id , path , parentElem , this , parentSvgDocId ) },
				function(){ that.#handleErrorResult(id,path,this,true) }
			);
	...
	//			console.log(" path:" + path);
				...
				var rPath = path;
				if ( this.#svgImagesProps[id].rootLayer && this.#svgImagesProps[this.#svgImagesProps[id].rootLayer].noCache ){
					rPath = UtilFuncs.getNoCacheRequest(rPath);
				}
				
				// 今後、ファイルのヘッダーにcommonQueryを埋め込むかもしれないためsvgImagesProps[id].commonQueryを設定しておきます
				const tempComQuery = this.#svgImagesProps[id].commonQuery || this.#svgImagesProps[this.#svgImagesProps[id].rootLayer]?.commonQuery;	// ?. : オプショナルチェーンという書き方でアクセス元がNullでも途中でエラーとならない書き方
				...
				if ( this.#mapViewerProps.uaProps.MS && httpObj.ontimeout ){ // MS(IEだけ？)のXHRはopen後にtimeoutを設定しないとエラーになる
		...
		if (( httpRes.readyState == 4 ) ){
			...
			if (httpRes.responseText.indexOf("http://www.w3.org/2000/svg")>=0){ // 2014.1.23 path以外もいろいろIEでは不具合が出るため、すべて対象に
				...
				if ( (resTxt.match(/data-controller-src([\s\S]*)"/ )) ){
					resTxt = UtilFuncs.getControllerSrc(resTxt,this.#svgImagesProps[docId]);
				}
				if ( (resTxt.match(/<script>([\s\S]*)<\/script>/ )) ){ // 2022/3/4
...
//				resTxt = resTxt.replace(/.*<!DOCTYPE html>.*</,'<'); // 2014.4.25 IE11 で怪しい挙動 <script>があると勝手にDOCTYPE htmlをつけているかんじがするぞ！！！
				this.#svgImages[docId] = new DOMParser().parseFromString(resTxt,"application/xml");
			} else { // このケースはなくすべき
				this.#svgImages[docId] = new DOMParser().parseFromString(httpRes.responseText,"application/xml");
			}
			...
			this.#svgImages[docId].getElementById = UtilFuncs.getElementByIdUsingQuerySelector; // added 2017.2.3
...
//							console.log("Detect attr change, delete parsed cache for : ",mutation.target);
			...
			if ( this.#svgImagesProps[parentSvgDocId] 
			&& this.#svgImagesProps[parentSvgDocId].childImages[docId] == SvgMapElementType.CLICKABLE ){
				this.#svgImagesProps[docId].isClickable = {value:true}; // 2023/11/27 boolean->obj
			}
			
			this.#setController( this.#svgImages[docId] , docPath , this.#svgImagesProps[docId] ); // 2016.10.14
			// ルートコンテナSVGのロード時専用の処理です・・・ 以下は基本的に起動直後一回しか通らないでしょう
			if ( docId =="root"){
				this.#mapViewerProps.rootCrs = this.#svgImagesProps[docId].CRS;
				this.#mapViewerProps.root2Geo = this.#matUtil.getInverseMatrix( this.#mapViewerProps.rootCrs );
				var viewBox = this.#getViewBox( this.#svgImages["root"] );
				this.#mapViewerProps.setRootViewBox( UtilFuncs.getrootViewBoxFromRootSVG( viewBox , this.#mapViewerProps.mapCanvasSize , this.#essentialUIs.ignoreMapAspect) );
			} else {
	...
	*/
	...
	//		console.log("called root dynamicLoad");
			...
			this.#essentialUIs.setGeoViewBox(this.#matUtil.getTransformedBox( this.#mapViewerProps.rootViewBox , this.#mapViewerProps.root2Geo ));
		...
		if ( docId == "root" ){
			...
			if ( this.#mapTicker.isEnabled() && !this.#mapTicker.pathHitTester.enable && !this.#geometryCapturer.GISgeometriesCaptureFlag){ // スマホなどでクリックしやすくするためのティッカー ただし単なるpathHitTestのときは無限ループが起きるのでパスする 2017.7.31 pathHitTest.enableチェックせずとも無限ループは起きなくなったはず 2018.1.18 GISgeometriesCapture中はtickerの表示は不要なので高速化のため外す2019.12.26
	...
	#handleScript( docId , zoom , child2root){
		this.#svgImagesProps[docId].script.location = UtilFuncs.getSvgLocation( this.#svgImagesProps[docId].Path ); // added 2017.9.5 ハッシュが書き換わる可能性を加味
		this.#svgImagesProps[docId].script.scale = zoom * child2root.scale;
		this.#svgImagesProps[docId].script.actualViewBox = this.#matUtil.getTransformedBox( this.#mapViewerProps.rootViewBox , this.#matUtil.getInverseMatrix( child2root ) ); // *ViewBoxは間違い・viewportが正しい・・互換のために残す・・・
		this.#svgImagesProps[docId].script.geoViewBox = this.#essentialUIs.geoViewBox;
		this.#svgImagesProps[docId].script.viewport = this.#svgImagesProps[docId].script.actualViewBox; // debug 2014.08.06
		this.#svgImagesProps[docId].script.geoViewport = this.#essentialUIs.geoViewBox; // debug
		...
		if ( vc =="zoom" || this.#svgImagesProps[docId].script.initialLoad ){ // zooming
		...
		if ( this.#svgImagesProps[docId].refresh.timeout > 0 && this.#svgImagesProps[docId].refresh.loadScript == true ){
	...
	#handlePreRenderControllerScript(docId , zoom , child2root, isSuperController){ // 2020/6/8  svg要素内のscriptをevalで実行するのをやめる前準備
	...
	#getLayerStatus(docId, zoom, child2root){ // 2022/03/11 開発中 この関数をLayerUIに渡すことで、.scriptの初期化処理(onloadFunc)を機能させることをもくろむ
		if ( !zoom || !child2root){
			var crs = this.#svgImagesProps[docId].CRS;
			child2root = this.#matUtil.getConversionMatrixViaGCS( crs, this.#mapViewerProps.rootCrs );
			var s2c = this.#getRootSvg2Canvas( this.#mapViewerProps.rootViewBox , this.#mapViewerProps.mapCanvasSize );
			zoom = this.#getZoom(s2c,docId);
		}
		...
		var svgDocStatus ={ // この辺の値は後々整理が必要
			...
			actualViewBox: this.#matUtil.getTransformedBox( this.#mapViewerProps.rootViewBox , this.#matUtil.getInverseMatrix( child2root ) ),
	...
	#viewBoxChanged=function(docId){ //  2020/6/8 修正 ただ、この関数、あまり筋が良いとは言えないので改修すべき・・
		...
		var ans;
		if ( !this.#prevRootViewBox[docId] || this.#prevRootViewBox[docId].width != this.#mapViewerProps.rootViewBox.width || this.#prevRootViewBox[docId].height != this.#mapViewerProps.rootViewBox.height ){
			ans = "zoom";
		} else if ( this.#prevRootViewBox[docId].x != this.#mapViewerProps.rootViewBox.x || this.#prevRootViewBox[docId].y != this.#mapViewerProps.rootViewBox.y ){
			ans = "scroll";
		} else {
			ans = false
		}
	...
	#parseSVG( svgElem , docId , parentElem , eraseAll , symbols , inCanvas , pStyle , dontChildResLoading ){ 
	...
	//	console.log("called parseSVG  id:",docId, "  Recursive?:",pStyle,"  dontRender?:",dontChildResLoading);
		...
		var beforeElem = null;
		var s2c = this.#getRootSvg2Canvas( this.#mapViewerProps.rootViewBox , this.#mapViewerProps.mapCanvasSize ); // ルートSVG⇒画面変換マトリクス
		var zoom = this.#getZoom(s2c,docId); // ルートSVGの、画面に対するズーム率 (docIdはレイヤーごとにdevicePixelRatioを変化させるための(副次的な)もの)
	...
	//	console.log(docPath);
		...
		var crs = this.#svgImagesProps[docId].CRS;
		if ( this.#svgImagesProps[docId].CRS.unresolved ){ // 2022/03/17
	//		console.log("Retry unresolved CRS resolution.");
		...
		var child2root = this.#matUtil.getConversionMatrixViaGCS( crs, this.#mapViewerProps.rootCrs );
	...
	//			console.log("svgElem:",svgElem.nodeName); // debug 下と同じ問題、ルートの要素を通過するときのみ呼ばないとまずいでしょ 2017.3.9
			...
			**/
			if ( typeof(this.#svgImagesProps[docId].preRenderControllerFunction)=="function" ){ // 2020/6/8  svg要素内のscriptをevalで実行するのをやめる前準備
	...
	//		console.log("node:" + i + "/" + svgNodes.length + " : " +svgNodes[i].nodeName);
			...
			case "iframe":
				...
				break;
			case "image":
				if ( UtilFuncs.getNonScalingOffset(svgNode).nonScaling ){ // 2018.3.2 imageでもnonScalingのものをPOIとする。getNonScalingOffsetをstyleパース時と都合二回呼んでるのがね。最初に読んでstyleのほうに受け渡したほうがキレイかと・・
	...
	//			console.log("group:",childCategory,childSubCategory);
			...
			if ( !this.#mapTicker.pathHitTester.enable && this.#geometryCapturer.GISgeometriesCaptureFlag ){
...
//				GISgeometry = SVGMapGISgeometry.createSVGMapGISgeometry( childCategory, childSubCategory , svgNode , this.#geometryCapturer.GISgeometriesCaptureOptions ); 
	...
	//			console.log("isLoadedImage:",imageId,imgElem);
				...
				var ip = UtilFuncs.getImageProps( svgNode , childCategory , pStyle , childSubCategory , GISgeometry); // x,y,w,h,href等読み込み
	...
	//			console.log("imageRect:",imageRect,  "  Elem:",svgNode,"  child2root:",child2root, "  geomProps:",ip,"  crs,rootCrs:",crs, rootCrs );
				...
				if ( imageRect.width==0 && imageRect.height==0 && (childCategory==SvgMapElementType.EMBEDSVG || childCategory == SvgMapElementType.BITIMAGE)){
				...
				if ( ip.commonQuery ){
					if(docId != "root"){ // commonQueryを保存するのはroot以外が対象
						this.#svgImagesProps[this.#svgImagesProps[docId].rootLayer].commonQuery = ip.commonQuery;
					}
				}
				...
				if ( !eraseAll && UtilFuncs.isIntersect( imageRect , this.#mapViewerProps.rootViewBox ) && UtilFuncs.inZoomRange( ip , zoom ,  imageRect.c2rScale ) && UtilFuncs.isVisible(ip ) ){ // ロードすべきイメージの場合
					...
					if ( ip.transform ){ // 2014.6.18 transform属性があるときの座標計算処理
						...
						if ( !cm.a ){ // added 2020/3/26 for non linear projection
							...
							var p2=this.#matUtil.transform( ip.x+ip.width , ip.y+ip.height , cm );
							var tMat = TernarySimultaneousEquationsSolution.getLinearTransformMatrix(
						...
						if ( childCategory == SvgMapElementType.POI && childSubCategory == SvgMapElementType.USEDPOI){ // ICON表示
					...
					if (!imgElem ){  // ロードされていないとき
						...
						var img;
						if ( childCategory == SvgMapElementType.POI || childCategory == SvgMapElementType.BITIMAGE ){ // image,use要素の場合
							var imageURL = UtilFuncs.getImageURL(ip.href,docDir);
							var isNoCache = (childCategory == SvgMapElementType.BITIMAGE && this.#svgImagesProps[docId].rootLayer && this.#svgImagesProps[this.#svgImagesProps[docId].rootLayer].noCache);
							...
							img.id = imageId;
							if ( ip.opacity ){
								if ( !this.#mapViewerProps.uaProps.MS){ // if ( !isIE)からチェンジ (Edge対策)
	...
	//								img.setAttribute("style" , "Filter: Alpha(Opacity=" + ip.opacity * 100 + ");opacity:" + ip.opacity + ";"); // IEではこれでは設定できない
							...
							if ( this.#svgImagesProps[docId].isClickable || (ip.elemClass && ip.elemClass.indexOf("clickable") >= 0 ) ){
								this.#svgImagesProps[docId].childImages[imageId] = SvgMapElementType.CLICKABLE;
							} else {
								this.#svgImagesProps[docId].childImages[imageId] = SvgMapElementType.EXIST;
							}
						}
						
						if ( childCategory == SvgMapElementType.POI && this.#geometryCapturer.GISgeometriesCaptureOptions.SkipVectorRendering ){
							// POIもベクタとして描画しない
						} else {
						...
						if ( this.#mapViewerProps.uaProps.isIE){ // IEではw,hの書き込みに失敗する場合がある。その対策。　imgエレメントのDOM登録タイミングによる？
							if ( this.#mapViewerProps.uaProps.verIE < 9 && (childCategory == SvgMapElementType.POI || childCategory == SvgMapElementType.BITIMAGE) ){ 
						...
						if ( childCategory == SvgMapElementType.POI || childCategory == SvgMapElementType.BITIMAGE ){ // image,use要素の場合
							...
							this.#imgRenderer.setImgElement(imgElem , xd.p0 , yd.p0, xd.span , yd.span , UtilFuncs.getImageURL(ip.href,docDir), elmTransform , 0, 0, false, ip.nonScaling, ip.href_fragment , ip.pixelated , ip.imageFilter, imageId , ip.opacity , ip.crossorigin, {docId:docId,svgNode:svgNode} ); // 2015.7.8 本来ip.cdxyは入れるべきだと思うが、どこかでダブルカウントされるバグがある

						} else if ( childCategory == SvgMapElementType.TEXT ){ // 2014.7.22
							...
							if ( this.#svgImagesProps[imageId] && this.#svgImagesProps[imageId].Path && this.#svgImagesProps[imageId].Path != childSVGPath ){
								...
								this.#svgImagesProps[imageId].Path = childSVGPath; // added 2017.9.5 : ハッシュが更新されることがあり、それを更新する必要があると思われる・・・　ISSUE:　ただ完全にURLが刷新されるケース(hrefがsetattributeされる)もあり、今のルーチンはそれを検出し再ロードできないのではないか？
							}
					...
					if ( childCategory == SvgMapElementType.POI ){ // 2018.3.2 変更はないが、use使わないがnonScalingのもの(DIRECTPOI)も追加
						this.#mapTicker.poiHitTester.setPoiBBox(imageId, xd.p0, yd.p0, xd.span, yd.span);
					}
					onViewport = true;
				...
				if ( svgNode.hasChildNodes() || childSubCategory == SvgMapElementType.SYMBOL){
	...
	#parseSVGwhenLoadCompleted(svgImages , imageId , imgElem , ct){
		if ( this.#svgImagesProps[imageId] && this.#svgImagesProps[imageId].loadError ){ // 2021/2/17 ERR404強化
			return;
		}
		if ( this.#svgImages[imageId] && this.#svgImagesProps[imageId] ){
			...
			if ( ct < 20 ){
				...
				setTimeout( 
					...
					}.bind(this) , 50 , svgImages , imageId , imgElem , ct );
			} else {
		...
		for ( var i = parentNode.childNodes.length - 1 ; i >= 0 ; i-- ){
			...
			if ( oneNode.nodeType == 1 ){
				if ( (oneNode.nodeName == "IMG" || oneNode.nodeName == "SPAN") && oneNode.id && oneNode.id.indexOf("toBeDel") == -1){ // 2018.2.23 text(はspanに入ってる)もimg同様にする
					if ( ( ! this.#existNodes[oneNode.id] ) ){ // img||text要素に対してのみ
	...
	#isLoadedImage( id ){ // HTMLのimg要素をサーチして、該当idがあるかどうかを確認する。(これが非効率な部分かも・・)
	...
	//	img.style.top = y + "px";
		if (!this.#mapViewerProps.uaProps.MS){
			img.style.bottom = ( this.#mapViewerProps.mapCanvasSize.height - ( y + cdy ) ) + "px";
		} else {
	...
	#hideAllTileImgs=function(){ // 2014.6.10 setGeoCenter,setGeoViewPortのちらつき改善
	...
	#getCrs( svgDoc ,docId){
		...
		try{
			var genericCRS ={ // 2022/03/17 CRSの関数がlayerUI、もしくはその中のscriptで定義されているケースに対応した　遅延実行処理
				unresolved:true,
				isSVG2:false
			};
			if ( globalView && globalView.nodeName =="view"){
				...
				if ( gcsElem ){
					...
					if ( tf ){
						...
						if (tf.indexOf("matrix")>=0){
						...
						} else if ( tf.toLowerCase()=="mercator"){ // 2020/3/24 add mercator special word support
							...
							var cntlWin = this.#svgImagesProps[this.#svgImagesProps[docId].rootLayer].controllerWindow;
							if ( cntlWin ){
						...
						} else if ( this.#svgImagesProps[docId].script && this.#svgImagesProps[docId].script.transformFunction){ // script要素の中のtransformFunction(layerUIのarrangeHtmlEmbedScriptが設定してる(この遅延処理に対応 2022/03/17)
	...
	#setController( svgDoc , docPath , svgImageProps){
		...
		if ( svgImageProps.controller &&  svgImageProps.controller.src ){ // data-controller-srcに直接ソースが書かれているケース
			if ( cntPath ){ // しかもdata-controllerにも書かれていたらそれをhashが書かれているとみなす
			...
			if ( cntPath ){
				var scurl = UtilFuncs.getImageURL(cntPath , UtilFuncs.getDocDir(docPath));
				...
				if(svgImageProps['parentDocId'] == 'root'){
					cntPath = this.#layerManager.getLayer(svgImageProps['rootLayer']).getAttribute('data-controller');
	...
	#getViewBox( svgDoc ){
		...
		if ( va ){
			if ( va.indexOf("#") == 0 ){
				...
				var rVB = this.#matUtil.getTransformedBox( globalVB , this.#mapViewerProps.rootCrs );
	...
	// まだrootSVGにのみ対応している・・
	#getZoom( s2c , docId){ // 2020/5/13 docId(というよりレイヤーID)によって、演算パラメータを変化させる機能を実装
		var ans ;
		var layerId = this.#svgImagesProps[docId].rootLayer;
		if ( this.#layerDevicePixelRatio[layerId]!= undefined){
			ans = ( Math.abs(s2c.a) + Math.abs(s2c.d) ) / ( 2.0 * this.#layerDevicePixelRatio[layerId] );
		} else {
			ans = ( Math.abs(s2c.a) + Math.abs(s2c.d) ) / ( 2.0 * this.#commonDevicePixelRatio );
		}
		return ( ans );
	...
	//	XMLhttpObject.withCredentials = true; // 認証情報をCORS時に入れる(ちょっと無条件は気になるが・・ CORSがワイルドカードだとアクセス失敗するので一旦禁止) 2016.8.23
		if ( timeoutFunc ){ // 2020/2/13 timeout処理機能を追加
			if ( !this.#mapViewerProps.uaProps.MS ){ // 2020/2/17 IEはエラーになるためopen後に実行する
	...
	#getSvgTarget( htmlImg ){
		...
		var ans = UtilFuncs.getElementByImgIdNoNS(this.#svgImages[svgDocId] , htmlImg.getAttribute("id"));
		return {
			element : ans,
			docId : svgDocId
		};
...
//				console.log( "NOW LOADING:: delay and retry refreshScreen" );
				...
				}.bind(this), 10); // 何度でもリトライし必ず実行することにする・・(問題起きるかも？)
...
//		eval( fname  ).bind(null,p1,p2,p3,p4,p5);
	...
	**/
	...
	captureGISgeometriesOption= function ( BitImageGeometriesCaptureFlg , TreatRectAsPolygonFlg , SkipVectorRenderingFlg ,  captureAlsoAsBitImage ){ // 2018.2.26
		// TBD : ロードできてないイメージは外すかどうか, onViewportのもののみにするかどうか のオプションもね
		if ( typeof BitImageGeometriesCaptureFlg == "object"){ // 第一オプションをオブジェクトにした場合は、ここで全部を設定できるようにする
			for ( var pn in BitImageGeometriesCaptureFlg){
				if (typeof BitImageGeometriesCaptureFlg[pn] == "boolean" &&  typeof this.#geometryCapturer.GISgeometriesCaptureOptions[pn]=="boolean"){
					this.#geometryCapturer.GISgeometriesCaptureOptions[pn]=BitImageGeometriesCaptureFlg[pn];
				}
			}
		} else { // バックワードコンパチのため・・・
			if ( BitImageGeometriesCaptureFlg === true || BitImageGeometriesCaptureFlg === false){
				this.#geometryCapturer.GISgeometriesCaptureOptions.BitImageGeometriesCaptureFlag = BitImageGeometriesCaptureFlg; // ビットイメージをキャプチャするかどうか
			}
			if ( TreatRectAsPolygonFlg === true || TreatRectAsPolygonFlg === false){
				this.#geometryCapturer.GISgeometriesCaptureOptions.TreatRectAsPolygonFlag = TreatRectAsPolygonFlg; // rect要素をPoint扱いにするかPolygon扱いにするか
			}
			if ( SkipVectorRenderingFlg === true || SkipVectorRenderingFlg === false ){
				this.#geometryCapturer.GISgeometriesCaptureOptions.SkipVectorRendering=SkipVectorRenderingFlg;
			}
			if ( typeof captureAlsoAsBitImage =="boolean"){
				this.#geometryCapturer.GISgeometriesCaptureOptions.captureAlsoAsBitImage = captureAlsoAsBitImage;
			}
		}
	}.bind(this);
	...
	**/
	getPoiPos(...params){ return (UtilFuncs.getNonScalingOffset(...params)) };
	getResume( ){ return ( this.#resumeManager.getResume()) };
	getRoot2Geo( ){ return (this.#mapViewerProps.root2Geo) };
